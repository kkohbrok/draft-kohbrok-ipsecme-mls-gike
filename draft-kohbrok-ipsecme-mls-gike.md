...
The existing KD payload is still reused as a container for MLS objects and Sender-ID material, so this profile does not allocate a separate IKE payload type for every MLS structure it carries.
...
This profile reuses KD bags for two purposes: KD Member Key Bags carry `GM_SENDER_ID` attributes and member-scoped MLS objects, and KD Group Key Bags carry group-scoped MLS objects associated with an epoch-bound Data-Security SA SPI.
...
| KD Group Key Bag | Retained for group-scoped MLS objects, but not for SA key delivery. |
| KD Member Key Bag | Retained for `GM_SENDER_ID` and member-scoped MLS objects. |
...
It creates a Commit that references the Proposal, includes an UpdatePath, creates a Welcome for the candidate, signs a GroupInfo for the new epoch without a `ratchet_tree` extension, and uploads the Commit, Welcome, and PartialGroupInfo to the GCKS as KD-carried MLS objects.
...
# KD-Carried MLS Objects and Exchanges

This profile does not define a separate IKEv2 payload type for each MLS object.
Instead, it reuses the KD payload defined in Section 4.5 of {{RFC9838}} and defines a multi-valued `MLS_OBJECT` attribute for both the Group Key Bag and Member Key Bag registries.
The attribute contains an object-type discriminator and an opaque object:
...
Member Key Bags carry member-scoped objects and objects that are not associated with a Data-Security SA SPI, such as KeyPackages, Welcomes, founder bootstrap state, and member uploads to the GCKS.
This use of KD carries no `SA_KEY`, `WRAP_KEY`, or `AUTH_KEY` material.
...
| GCKS fan-out of an ordered path-refresh `mls_commit` with the epoch-bound GSA | Group Key Bag |
